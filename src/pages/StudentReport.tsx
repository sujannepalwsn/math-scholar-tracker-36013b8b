...
export default function StudentReport() {
  ...
  const [selectedStudentId, setSelectedStudentId] = useState<string>(searchParams.get("studentId") || "none");
  ...
  const { data: publishedExams = [] } = useQuery({
    queryKey: ["published-exams-report", user?.center_id, gradeFilter, user?.role, user?.teacher_id],
    queryFn: async () => {
  ...
  const { data: allExamSubjects = [] } = useQuery({
    queryKey: ["all-exam-subjects-report", user?.center_id],
    queryFn: async () => {
      if (!user?.center_id) return [];
  ...
  const { data: examMarks = [], isLoading: isExamMarksLoading } = useQuery({
    queryKey: ["student-exam-marks-report", selectedStudentId, gradeFilter, selectedExamId, dateRange, studentIds],
    queryFn: async () => {
  ...
  const isLoading = isAttendanceLoading || isChaptersLoading || isTestsLoading || isHomeworkLoading || isActivitiesLoading || isDisciplineLoading || isAllActivitiesLoading || isExamMarksLoading;
                                ...
                                isSingleStudent && studentData ? (
                                  ...
                                  <div className="flex flex-col">
                                    <span className="text-primary">
                                      {Object.values(studentResultsMap).filter(r => r.passed).length}/{Object.keys(studentResultsMap).length} Passed
                                    </span>
                                    <span className="text-[10px] text-muted-foreground font-medium">
                                      Class Avg: {(Object.values(studentResultsMap).reduce((acc, r) => acc + (r.obtained/r.full), 0) / Object.keys(studentResultsMap).length * 100).toFixed(1)}%
                                    </span>
                                  </div>
                                )
                              ) : (
                            ...
                            <td className="px-6 py-4 text-center">
                              {isSingleStudent && studentData ? (
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="h-8 w-8 text-primary hover:text-primary/80 hover:bg-primary/10 rounded-full"
                                  onClick={() => navigate(user?.role === 'teacher' ? `/teacher/results-dashboard?examId=${exam.id}` : `/results-dashboard?examId=${exam.id}`)}
                                >
